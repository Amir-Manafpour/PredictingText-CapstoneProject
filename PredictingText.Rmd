...
freqMat <- getFreqMatrix(docsOrig, maxngram = 3, coverage = 1.0)
pmat <- getProbMatrix(freqMat)

set.seed(8425)
testSubset <- getImportDt(linesnmax = linesnmax, nsamp = nsamp)
testDocs <- dtToQcorp(testSubset)
...
getScoreMatrix <- function(trainProbMat, testDocs) {
  ...
  ngramTestTermsDt <- getOrderedFreqDt(Get_qdfm(testDocs, n = nmax), spellCheck = T)
  ngramTestTermsDt[, score:=numeric()]
  for (row in 1:nrow(ngramTestTermsDt)) {
    ngramTestTermsDt[row, score:=trainFreqdt[term == term]]
    rowTerm <- ngramTestTermsDt$term[row]
    for (n in nmax:2){
      trainProbDt <- trainProbMat[[n]]
      termProb <- trainProbDt[term == rowTerm, p]
      if(is.na(trainProbDt))
        ngramTestTermsDt$score[row] <- trainProbDt$p[term == rowTerm]
    }
  }
  

  
  # Look for highest n-gram matches first
  
  # Add score column to trainFreqMat
  
  # Loop down to bigram multiplying score by 0.4 each time
