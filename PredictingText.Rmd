...
```{r modeling}
# Clean corpus including stopwords and tokenize
...
freqdt <- data.table(term = names(freqWordsSW), freq = freqWordsSW)

freqdt_bi <- data.table(term = names(freqBigramSW), freq = freqBigramSW)
head(separate(freqdt_bi, term, NA, sep = " "))

wrds <- sapply(sapply(names(freqBigramSW), strsplit, split = " "), unlist)
